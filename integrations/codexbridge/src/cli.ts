...
import { WecomPlatformPlugin } from './platforms/wecom/plugin.js';
...
async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (group === 'wecom' && command === 'serve') {
    return runWecomServe(args);
  }
...
async function runWecomServe(args: string[]) {
  const i18n = createI18n();
  const options = parseWecomServeArgs(args);
  const stateDir = path.resolve(options.stateDir ?? defaultCodexBridgeStateDir());
  const defaultCwd = path.resolve(options.cwd ?? process.env.CODEXBRIDGE_DEFAULT_CWD ?? process.cwd());
  const botId = options.botId ?? normalizeCliString(process.env.WECOM_BOT_ID);
  const secret = options.secret ?? normalizeCliString(process.env.WECOM_SECRET);
  const corpId = options.corpId ?? normalizeCliString(process.env.WECOM_CORP_ID);
  const maxMessageLength = options.maxMessageLength
    ?? parseOptionalNonNegativeInt(process.env.WECOM_MAX_MESSAGE_LENGTH)
    ?? 4000;
  if (!botId) {
    throw new Error('Missing WECOM_BOT_ID / --bot-id');
  }
  if (!secret) {
    throw new Error('Missing WECOM_SECRET / --secret');
  }

  const serveLock = await acquireServeLock(path.join(stateDir, 'runtime', 'wecom-serve.lock'));
  const repositories = createFileJsonRepositories(path.join(stateDir, 'runtime'));
  const codexProfiles = loadCodexProfilesFromEnv();
  const codexAuthManager = createWeixinServeCodexAuthManager(stateDir);
  const runtime = createCodexBridgeRuntime({
    platformPlugins: [
      new WecomPlatformPlugin({
        botId,
        secret,
        corpId,
        cwd: defaultCwd,
        maxMessageLength,
        logger: {
          debug: (message: unknown, ...args: unknown[]) => process.stderr.write(`[wecom:debug] ${String(message)} ${args.map(formatError).join(' ')}\n`),
          info: (message: unknown, ...args: unknown[]) => process.stderr.write(`[wecom:info] ${String(message)} ${args.map(formatError).join(' ')}\n`),
          warn: (message: unknown, ...args: unknown[]) => process.stderr.write(`[wecom:warn] ${String(message)} ${args.map(formatError).join(' ')}\n`),
          error: (message: unknown, ...args: unknown[]) => process.stderr.write(`[wecom:error] ${String(message)} ${args.map(formatError).join(' ')}\n`),
        },
      }),
    ],
    providerPlugins: [
  ...
  const platformPlugin = runtime.registry.getPlatform('wecom') as WecomPlatformPlugin;
  const bridgeRuntime = new WeixinBridgeRuntime({
    platformPlugin,
    bridgeCoordinator: runtime.services.bridgeCoordinator,
    automationJobs: runtime.services.automationJobs,
    ...
    assistantRecords: runtime.services.assistantRecords,
    onError: (async (error: unknown) => {
      process.stderr.write(`[wecom] ${formatError(error)}\n`);
    }) as any,
    locale: i18n.locale,
  } as any);
  const embeddedNativeApiOptions = resolveEmbeddedCodexNativeApiOptions({
  ...
  process.stdout.write('Enterprise WeChat bridge starting\n');
  process.stdout.write(`state_dir: ${stateDir}\n`);
  process.stdout.write(`default_provider_profile: ${runtime.config.defaultProviderProfileId}\n`);
  process.stdout.write(`serve_lock: ${serveLock.lockPath}\n`);
  process.stdout.write(`default_cwd: ${runtime.config.defaultCwd ?? i18n.t('common.none')}\n`);
  process.stdout.write(`wecom_bot_id: ${botId}\n`);
  process.stdout.write(`wecom_corp_id: ${corpId ?? ''}\n`);
  process.stdout.write(`native_api_enabled: ${nativeApi ? 'true' : 'false'}\n`);
  ...
  const stop = async (signal: string) => {
    if (stopped) return;
    stopped = true;
    process.stdout.write(`Enterprise WeChat bridge stopping: ${signal}\n`);
    try {
      await bridgeRuntime.stop();
      await nativeApi?.stop().catch(() => {});
    } finally {
...
function parseWecomServeArgs(args: string[]): WecomServeArgs {
  const options: WecomServeArgs = {
    stateDir: null,
    cwd: null,
    botId: null,
    secret: null,
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === '--max-message-length' && next) {
      options.maxMessageLength = parseOptionalNonNegativeInt(next);
      index += 1;
    }
  }
  return options;
