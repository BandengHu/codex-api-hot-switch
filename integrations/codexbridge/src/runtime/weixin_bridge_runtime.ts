...
interface PlatformPluginLike {
  ...
  beginStream?(params: { externalScopeId: string }): Promise<PlatformStreamSession | null | undefined> | PlatformStreamSession | null | undefined;
...
interface StreamState {
  ...
  // Real streaming (single-bubble cumulative updates), used when the platform exposes beginStream (e.g. Enterprise WeChat).
  liveStream: PlatformStreamSession | null;
  liveStreamReady: boolean;
  liveStreamInitPromise: Promise<void> | null;
  liveStreamFailed: boolean;
  liveText: string;
...
export class WeixinBridgeRuntime {
  ...
  constructor({
    ...
    this.platformId = typeof platformPlugin?.id === 'string' && platformPlugin.id.trim()
      ? platformPlugin.id.trim()
      : 'weixin';
  ...
  async processInboundEventWithOptions(
    ...
    // For platforms with real single-bubble streaming, open the bubble immediately with a placeholder
    // so the user sees activity even before the first model token arrives.
    if (!options.suppressProgressDelivery && this.supportsLiveStreaming()) {
      streamState.liveText = this.i18n.t('runtime.stream.thinking');
      streamState.liveTextIsPlaceholder = true;
    ...
    try {
      ...
      // Surface CLI/Codex failures to the user instead of leaving them with silence.
      const message = this.i18n.t('runtime.error.unexpected', { error: formatAutomationError(error) });
      let delivered = false;
      if (streamState.liveStreamReady && !streamState.liveStreamFailed) {
        delivered = (await this.finalizeLiveStream(streamState, message)) !== null;
      }
      if (!delivered) {
        await this.stopPreviewStreaming(streamState);
        await this.sendTextWithRetry({ externalScopeId: event.externalScopeId, content: message }).catch(() => {});
      }
      throw error;
    } finally {
      await typingStart;
  ...
  async handleProgressUpdate(
    ...
    if (this.supportsLiveStreaming()) {
      if (streamState.liveTextIsPlaceholder) {
  ...
  supportsLiveStreaming(): boolean {
    return typeof this.platformPlugin.beginStream === 'function';
  }

  async ensureLiveStreamInitialized(event: InboundTextEvent, streamState: StreamState): Promise<void> {
    if (streamState.liveStreamReady || streamState.liveStreamFailed) {
      return;
    }
    if (streamState.liveStreamInitPromise) {
      await streamState.liveStreamInitPromise;
      return;
    }
    streamState.liveStreamInitPromise = (async () => {
      try {
        const session = await this.platformPlugin.beginStream?.({ externalScopeId: event.externalScopeId });
        if (session) {
          streamState.liveStream = session;
          streamState.liveStreamReady = true;
        } else {
          streamState.liveStreamFailed = true;
        }
      } catch (error) {
        streamState.liveStreamFailed = true;
        debugRuntime('live_stream_init_failed', {
          scopeId: event.externalScopeId,
    ...
    await streamState.liveStreamInitPromise;
    streamState.liveStreamInitPromise = null;
  }

  ensureLiveStreamPush(event: InboundTextEvent, streamState: StreamState): void {
    if (streamState.liveStreamPushPromise || streamState.previewStopped || streamState.streamingDisabled || streamState.liveStreamFailed) {
      return;
    }
    streamState.liveStreamPushPromise = this.runLiveStreamPush(event, streamState)
      .finally(() => {
        streamState.liveStreamPushPromise = null;
      });
  }

  async runLiveStreamPush(event: InboundTextEvent, streamState: StreamState): Promise<void> {
    await this.ensureLiveStreamInitialized(event, streamState);
    if (!streamState.liveStreamReady || !streamState.liveStream || streamState.liveStreamFailed) {
      // Streaming unavailable: fall back to the chunked preview path with whatever has accumulated.
      this.fallbackLiveToPreview(event, streamState);
      return;
    }
    const text = streamState.liveText.trim();
    if (!text) {
      return;
    }
    const ok = await streamState.liveStream.push(text);
    if (!ok) {
      streamState.liveStreamFailed = true;
      this.fallbackLiveToPreview(event, streamState);
    }
  ...
  fallbackLiveToPreview(event: InboundTextEvent, streamState: StreamState): void {
    if (streamState.liveTextIsPlaceholder) {
      // Nothing real has streamed yet; drop the placeholder so it is not re-sent as a chunk.
      streamState.liveText = '';
      streamState.liveTextIsPlaceholder = false;
      return;
    }
    // Move not-yet-rendered live text into the chunk pump so nothing is lost when streaming is unavailable.
    const remaining = streamState.liveText.slice(streamState.streamedText.length);
    streamState.liveText = '';
    if (remaining.trim()) {
      streamState.pendingPreview += remaining;
      this.ensurePreviewPump(event, streamState);
    }
  }
  ...
  async stopPreviewStreaming(streamState: StreamState): Promise<void> {
    ...
    const livePush = streamState.liveStreamPushPromise;
    if (livePush) {
      await livePush;
    }
  }

  // Finalize the single-bubble live stream with the final cumulative text.
  // Returns the delivered content when handled here, or null to let the caller fall back to sendText.
  async finalizeLiveStream(streamState: StreamState, finalText: string): Promise<string | null> {
    if (!streamState.liveStreamReady || !streamState.liveStream || streamState.liveStreamFailed) {
      return null;
    }
    ...
    streamState.streamedText = content;
    return content;
  }
  ...
  async ensureFinalDelivered(
    ...
    // Live single-bubble streaming path (e.g. Enterprise WeChat): finalize the same bubble for every terminal state.
    if (streamState.liveStreamReady && !streamState.liveStreamFailed) {
      // Media-only completions have no text body; abandon the live bubble and let the media path handle delivery.
      const mediaOnly = !normalizedFinal && artifactMessages.length > 0 && outputState === 'complete';
      if (!mediaOnly) {
        const failureMessage = outputState === 'complete'
          ? ''
          : errorMessage
        ...
        const liveFinalText = normalizedFinal
          ? (failureMessage ? `${finalText}\n\n${failureMessage}` : finalText)
          : failureMessage;
        if (String(liveFinalText ?? '').trim()) {
          const delivered = await this.finalizeLiveStream(streamState, liveFinalText);
          if (delivered !== null) {
            debugRuntime('final_delivery_decision', {
              scopeId: event.externalScopeId,
              outputState,
              finalSource: codexTurnMeta?.finalSource ?? null,
              completionMode: 'live_stream_finish',
              deliveryContent: truncateDebugText(delivered),
            });
            return {
              source: codexTurnMeta?.finalSource ?? (outputState === 'complete' ? 'thread_items' : 'progress_only'),
              mode: outputState === 'complete' ? 'live_stream_finish' : 'live_stream_finish_failure',
              finalText,
              sentContent: delivered,
              delivered: true,
              rateLimited: false,
              error: outputState === 'complete' ? null : (errorMessage || null),
            };
          }
          // finish() failed: fall through to the normal sendText-based delivery below.
        }
      } else {
        // Abandon the live bubble so the dangling "thinking…" frame does not linger before media delivery.
        streamState.liveStream?.abort();
        streamState.liveStreamFailed = true;
      }
    }
  ...
  async runAutomationSweepInternal(): Promise<void> {
    const automationJobs = this.automationJobs?.claimDueJobs?.(this.platformId) ?? [];
    ...
    if (isAgentCommandEnabled()) {
      const agentJobs = typeof this.agentJobs?.claimSupervisableJobs === 'function'
        ? this.agentJobs.claimSupervisableJobs(this.platformId)
        : this.agentJobs?.claimQueuedJobs?.(this.platformId) ?? [];
    ...
    const reminders = this.assistantRecords?.listDueReminders?.(this.platformId)
      ?? this.assistantRecords?.claimDueReminders?.(this.platformId)
      ?? [];
  ...
  async runAgentJob(job: any): Promise<RuntimeResponse> {
    ...
    const event: InboundTextEvent = {
      platform: String(job.platform ?? this.platformId),
      externalScopeId: scopeId,
