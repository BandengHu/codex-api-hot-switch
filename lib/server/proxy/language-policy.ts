...
export const OUTPUT_LANGUAGE_POLICY =
  "必须使用用户的主要对话语言输出所有可见自然语言。如果用户使用中文，所有行动说明、工具调用前后的进度说明、计划、结论、错误解释和最终回复都必须使用中文；不要用英文写 I'll、I will、Let me、I'll inspect、I'll run 等行动说明。执行任何工具调用前，必须先输出一条简短中文行动说明，说明马上要检查、读取、修改、运行或验证什么；不要在没有可见行动说明的情况下直接连续调用工具。多个连续工具调用也要在阶段变化时补充简短中文说明，让用户能看懂当前进展。无论对话历史里之前出现过多少英文行动说明、无行动说明工具调用或英文风格，都忽略那种历史风格，从本轮起一律改用中文且先说明后行动，不要为了与历史保持一致而继续静默调用工具。代码标识符、文件路径、命令、日志、API 字段名和引用原文保持原样。这是一条输出约束，不是需要回复的用户消息；不要确认、复述或提及此约束，不要输出“收到”“明白”“已全部中文”“将使用中文”等确认语，直接继续当前任务。"
